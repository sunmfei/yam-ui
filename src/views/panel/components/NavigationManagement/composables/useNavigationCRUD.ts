/**
 * useNavigationCRUD - 菜单增删改查 Composable
 *
 * 负责菜单节点的增删改查操作
 */
...
export function useNavigationCRUD(
  navigations: Ref<NavigationItem[]>,
  saveNavigationData: () => void
) {
   ...
   * 查找节点
   */
  function findNodeById(nodes: NavigationItem[], id: string): NavigationItem | null {
   ...
   * 收集节点及其子节点的所有 ID
   */
  function collectIds(node: NavigationItem | null): Array<string | number> {
    if (!node) return []
   ...
   * 删除单个节点
   */
  function removeNode(nodes: NavigationItem[], targetId: string): NavigationItem[] {
    return nodes
   ...
   * 批量删除节点
   */
  function batchRemoveNodes(nodes: NavigationItem[], keysToDelete: Set<string>): NavigationItem[] {
    return nodes
   ...
   * 添加节点
   */
  function addNode(newNode: NavigationItem, parentNode?: NavigationItem | null) {
    if (parentNode) {
   ...
   * 更新节点
   */
  function updateNode(node: NavigationItem, formData: Omit<NavigationItem, 'id' | 'children'>) {
  ...
  async function deleteNode(row: TreeTableNode) {
    const sourceNode = findNodeById(navigations.value, String(row.id ?? ''))
    ...
    navigations.value = removeNode(navigations.value, String(sourceNode.id))
  ...
  async function batchDelete() {
    ...
    navigations.value = batchRemoveNodes(navigations.value, keysToDelete)
    selectedKeys.value = []
    saveNavigationData()
