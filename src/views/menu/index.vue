    ...
    <section class="rounded-3xl border border-border/60 bg-card p-6 shadow-sm">
      <div class="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div class="space-y-3">
          ...
          <div>
            <h1 class="text-3xl font-semibold tracking-tight">菜单树形表格</h1>
        ...
        <div class="flex flex-wrap gap-2">
          <Button variant="outline" @click="handleReset">重置数据</Button>
          <Button variant="outline" @click="handleExport">导出 JSON</Button>
      ...
      <div class="mt-6 grid gap-4 lg:grid-cols-4">
        <div class="rounded-2xl border border-border/60 bg-background/70 p-4">
          <div class="text-xs uppercase tracking-[0.14em] text-muted-foreground">节点总数</div>
          <div class="mt-2 text-2xl font-semibold">{{ totalCount }}</div>
        </div>
        <div class="rounded-2xl border border-border/60 bg-background/70 p-4">
          <div class="text-xs uppercase tracking-[0.14em] text-muted-foreground">过滤后节点</div>
          <div class="mt-2 text-2xl font-semibold">{{ filteredCount }}</div>
        </div>
        <div class="rounded-2xl border border-border/60 bg-background/70 p-4">
          <div class="text-xs uppercase tracking-[0.14em] text-muted-foreground">已选中</div>
          <div class="mt-2 text-2xl font-semibold">{{ selectedKeys.length }}</div>
        </div>
        <div class="rounded-2xl border border-border/60 bg-background/70 p-4">
          <div class="text-xs uppercase tracking-[0.14em] text-muted-foreground">模式</div>
          <div class="mt-2 text-sm font-semibold">
            {{ treeEnabled ? '树形表格模式' : '扁平表格模式' }}
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="rounded-3xl border border-border/60 bg-card p-6 shadow-sm">
      <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div class="space-y-4">
          ...
          <div class="flex flex-col gap-4 lg:flex-row">
            ...
            <div class="relative flex-1">
              ...
              />
              <Input v-model="keyword" class="pl-9" placeholder="搜索名称、路径或操作键" />
            </div>

            <!-- 类型筛选 -->
            <select
              v-model="selectedType"
              class="h-9 min-w-45 rounded-lg border border-input bg-background px-3 text-sm outline-none transition focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
            >
          ...
          >
            ...
            <template #status="{ row }">
              <div class="flex flex-wrap gap-2">
                <Badge v-if="row.hidden" variant="outline">隐藏</Badge>
                <Badge v-if="row.disabled" variant="warning">禁用</Badge>
                <Badge v-if="!row.hidden && !row.disabled" variant="secondary">正常</Badge>
              </div>
            </template>
            ...
            <template #actions="{ row }">
              <div class="flex items-center justify-end gap-2">
                <Button size="sm" variant="outline" @click.stop="handleAddChild(row)">
                  添加子节点
                </Button>
        ...
        <aside class="space-y-4 rounded-2xl border border-border/60 bg-background/70 p-5">
          <div>
            <h2 class="text-base font-semibold">配置面板</h2>
            <p class="mt-1 text-sm text-muted-foreground">修改下方开关可实时更新组件配置</p>
          </div>
          ...
          <div class="space-y-4">
            ...
            >
              <div>
                <div class="text-sm font-medium">启用树形模式</div>
                <div class="text-xs text-muted-foreground">关闭后显示为扁平表格</div>
              </div>
            ...
            >
              <div>
                <div class="text-sm font-medium">默认展开全部</div>
                <div class="text-xs text-muted-foreground">页面加载时展开所有节点</div>
              </div>
            ...
            >
              <div>
                <div class="text-sm font-medium">点击行展开</div>
                <div class="text-xs text-muted-foreground">点击整行触发展开/收起</div>
              </div>
            ...
            >
              <div>
                <div class="text-sm font-medium">启用多选</div>
                <div class="text-xs text-muted-foreground">显示复选框列</div>
              </div>
            ...
            >
              <div>
                <div class="text-sm font-medium">严格选择模式</div>
                <div class="text-xs text-muted-foreground">父子节点不联动选择</div>
              </div>
            ...
            >
              <div>
                <div class="text-sm font-medium">启用分页</div>
                <div class="text-xs text-muted-foreground">使用内置分页器</div>
              </div>
            ...
            <div class="rounded-xl border border-border/60 px-4 py-3">
              <div class="text-sm font-medium">每页条数</div>
          ...
          <div v-if="activeRow" class="rounded-xl border border-border/60 bg-card p-4">
            <div class="text-sm font-medium">当前激活行</div>
