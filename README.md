...
这样前端只请求同源 `/api` 和 `/auth`，其中 `/auth/*` 会直接代理到认证服务的 `/auth/*`，浏览器 Cookie 的 `Path=/auth` 也能自然复用。

### OAuth2/OIDC 联调与跨域

- 前端登录表单先调用同源 `/auth/login`，由代理转发到认证服务建立会话。
- 登录成功后再跳转到 `/auth/oauth2/authorize` 获取授权码。
- 前端换 token、刷新 token、获取用户信息统一走 `/auth/*`，由 Vite 代理到认证服务。
